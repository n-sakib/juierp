...
function dbEcho($query)
{   
	echo "$query<br>"; //for debuggin purpose
...
function uploadPhoto($htmlName, $newFileName)
{
	...
	if ($_FILES[$htmlName]["error"] > 0)
       {echo "Error: " . $_FILES[$htmlName]["error"] . "<br>";}
}
function uploadPhotoIn($htmlName, $filePathWithoutExtension)
{
	// initial data fetching
	$name=	$_FILES[$htmlName]['name'];
	$tempName=	$_FILES[$htmlName]['tmp_name'];
	...
	$path = dirname(__FILE__)."/../../media/image/$filePathWithoutExtension". $ext;
	//$path = "/../../media/image/product/shoe/". $fileName . $ext;
	echo "$path";
...
function uploadPhotosIn($htmlName, $index, $filePathWithoutExtension)
{
	// initial data fetching
	$name=	$_FILES[$htmlName]['name'][$index];
	$tempName=	$_FILES[$htmlName]['tmp_name'][$index];
	$type=	$_FILES[$htmlName]['type'][$index];
	$size=	$_FILES[$htmlName]['size'][$index];
	...
	$path = dirname(__FILE__)."/../../media/image/$filePathWithoutExtension". @$ext; //automatically places empty file in array when empty file input is passed @$ext is error ignored cause, throws exception in such case
	//$path = "/../../media/image/product/shoe/". $fileName . $ext;
	//echo "$path";
	...
	if ($_FILES[$htmlName]["error"][$index] > 0)
       {	//print_r($_FILES);
       	//echo "Error: " . $_FILES[$htmlName]["error"][$index]. "<br>";
       }
}
 ?>